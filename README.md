...
The light dashboard has four views: **Overview** (workers, queue, progress and task output), **Tasks**, **Run history**, and **Settings**. Settings shows each worker's cached agent authentication, CLI versions, latest recorded launch settings, and native model and effort defaults from the worker's user settings. Empty model values display as `CLI default`; verified effort choices are editable, while an existing unverified effort remains visible with `Choices unavailable` and can be cleared to `CLI default` before changing its model. OpenCode does not publish a global effort setting. Saving uses an optimistic revision check, preserves the draft on errors or conflicts, and affects subsequent launches; task, environment, project, and model-specific settings may override it. Stale or missing observations show an unknown connection state. Fonts are bundled locally under the SIL Open Font License.
