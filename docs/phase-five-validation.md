...
This record covers the fresh v6 live acceptance against the execution-core revision. Attempts 1 to 4, including the superseded v4 and v5 roots, are not evidence for this run; their tasks were closed or abandoned before the corrected v6 run. The v6 setup gate reached three ready workers with authenticated Codex 0.153.2. No profile was created, uploaded, printed, or used.
...
The v6 run used a fresh isolated root and a copied three-worker inventory. Claude Code is deferred by operator decision; Codex is the only agent for this run. The dashboard tasks view was observed separately; source = origin, publish = push, Cursor, OpenCode, and worker gc remain later phases.

| Required evidence | Sanitized record |
| --- | --- |
| Live-task source commit | `ad15b09`. The v6 task executions used the isolated source clone at this revision. |
...
| Helper/client revision match | Match on all three workers during v6 and again after the final post-rebase setup. The installed helper digest equaled the corresponding release client digest; installation locations are omitted. |
...
| 2. needs_input then say | Task `59f90ef6` on mini-1: the first turn ended `needs_input` with a session present. Three resume attempts were lost; same-session completion was not proven. Sanitized symptom: “the worker-side Codex process panicked on a closed stdout pipe during resume turns and the turn was recorded lost; I am investigating that separately”. |
| 3. cancel during a turn, then say | Task `57f4ad3f` on mini-3: the first turn was cancelled, the resume turn was lost, and no completion was published. Cancel-then-resume completion was not proven. Sanitized symptom: “the worker-side Codex process panicked on a closed stdout pipe during resume turns and the turn was recorded lost; I am investigating that separately”. |
...
| 9. Retained metadata privacy | Count-only scans: local marker `0`, secret `0`, path-like hits `62`; mini-1 marker `0`, secret `0`, path-like hits `159`; mini-2 `0`, `0`, `72`; mini-3 `0`, `0`, `55`. Marker/secret retention passed; the path-count criterion did not. |
