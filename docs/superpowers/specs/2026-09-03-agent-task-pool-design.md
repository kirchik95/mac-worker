...
Detached runner startup uses the configured worker count as its process budget and counts unique live process identities. Excess turns are parked without a process. When a detached runner cannot claim its waiting turn, it may reuse its PID for an eligible parked turn on an idle worker: under the queue/state lock it parks its own row, dispatches the recipient, publishes recipient runner metadata, and clears donor metadata. Both rows keep their original queue positions and pins. Attached execution stays bound to its requested task. Terminal completion and mutating task commands can also start parked turns. A waiting runner polls with exponential backoff from one second to thirty seconds and records fleet observations in the shared admission cache.

Submission saves the originating worktree in immutable, owner-only `turns/<task_id>/project.json`, bound to the task/project/worktree/repository identities. Runners and task operations use this context for execution, cleanup and result import independently of the caller's cwd. Legacy records retain a cwd fallback; reassignment across worktrees requires saved context. Public task and queue records do not contain the local path. See [waiting runner reassignment](2026-09-08-runner-reassignment-design.md) for publication and recovery boundaries.
...
The phase 4 queue is extended with an entry `kind` (`batch`, `task_turn`), an optional hard pin, and an optional run reference. Admission is per-worker FIFO as amended in section 5.1: rows that no idle worker can take do not block younger rows for other workers. First turns rank by worktree affinity, project affinity, memory, disk, and name. Follow-up turns have a hard pin. Normal task claims are scoped to both owner and turn; older eligible waiting rows with live owners and executable parked rows retain priority on that worker. Detached runner reassignment can atomically adopt and dispatch an eligible parked turn, subject to the same reservations and run cap. A displaced donor keeps its queue position, so parked rows can be older than owned waiting rows. Incomplete submission/rollback records, terminal tasks, and parked rows without executable prompts are excluded from parked-task selection and FIFO barriers.
