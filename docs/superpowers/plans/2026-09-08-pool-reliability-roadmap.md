...
Дальше, не этот этап: довести изоляцию дескрипторов в fork-тестах. **`release.yml` alignment with current CI (macos-15, checkout@v7, Node 22 UI gates, no `--test-threads=1`) is owned by the 2026-09-10 SCHED increment and is not claimed done here until that commit SHA is recorded.** Remote snapshot race coverage is in `8f10ed2`. Источники PR: [hosted runners](https://docs.github.com/en/actions/reference/runners/github-hosted-runners), [runner-images](https://github.com/actions/runner-images/blob/main/README.md), [checkout](https://github.com/actions/checkout), [setup-node](https://github.com/actions/setup-node). Семнадцать отложенных замечаний review не закрыты. Этап 5 — только после измерений; ускорений кода здесь нет.
...
**Прогресс 2026-09-09 — пункт 2, admission cache до SSH, не весь этап 5.** Реализация на `admission-cache-20260909`. Парные измерения одинаковым V2 harness (`ccc87bdc…`) на `8f10ed2936d1156c6d8631f0efb61de15d6cad7d` → `7519441fe3da6a45ef4e01a7b511efa55dfe1586`. Комбинированный runtime `96ccc2e356a547ff3ce99052995304617bd3de03` (merge exact main `1db117d`, Herdr count через shared admission) **не** переизмерялся. Production logic с `96ccc2e`; test-only `ee6cec3` / `1c06046` / `0745f2b`. Полный `fmt` 0 / `--all-targets` 0 (585 с, 68 `Running`, **1794** passed / 0 failed / 0 ignored) / Clippy `-D warnings` 0 на `0745f2b` (`validation/final-retry2/`). Первые два полных прогона красные: `96ccc2e` 823 passed / 1 failed; `ee6cec3` lib 342 passed / 1 failed. Локальный fake 50 ms transport, n=5, debug/test, busy host. Counts стабильны (n5 peak 1→3 при 5 probes; runner-after 1→0; offline 4→2); wall — наблюдение, не guaranteed fleet speedup. Несколько warm/repeated/n1/stale/overlap medians выросли. `QUEUE_TIME_REGRESSION` на ungated concurrent enqueue не чинился. Подробности, все 9 сценариев и пределы: [2026-09-09-admission-performance.md](../../2026-09-09-admission-performance.md).

**Прогресс 2026-09-09 — пункт 3 частично (Stage 5.3a): no-op task writes и non-overlapping UI polls, не весь пункт 3 и не весь этап 5.** Реализация и полный gate на `915227f354a847a1dabab453ca35d528493db567` поверх exactbase `e8c1bb24db30aadf3e7f54ce5933cf1fd3d1087b`. Обычные идентичные `LocalTaskRecord` пропускают atomic replacement после recover/parse/identity под lock (inode 5→0 на пяти identical `update_task`); expected-record CAS только у `update_task_if_current`. Changed `status_observed_at` остаётся 5; residue plant+identical retry 2→1; corrupt по-прежнему reject; `clear_submission_intent` остаётся Replace (2→2 на публичном harness). Runner handoff тоже Replace, но этим inode-harness не измерялся (source review / полный suite). TaskDetail/`useSnapshot`: один in-flight запрос на generation, старт сразу, следующий poll через 2 с после settle; pending 6000 ms requests/live peak 4→1. Point lookup по ID уже в [2026-09-09-performance-improvements.md](../../2026-09-09-performance-improvements.md). Сбор observations независимо от HTTP **не** в этом инкременте. Полный `fmt` 0 / `--all-targets` 0 (610.342 с, 68 `Running`, **1812** passed / 0 failed / 0 ignored; три nested helper исключены) / Clippy `-D warnings` 0; UI 105 tests / 12 files; lint/build/`diff -r`/`git diff --check` 0. Это счётчики replacement/request, не fleet latency. Подробности: [2026-09-09-task-write-polling-performance.md](../../2026-09-09-task-write-polling-performance.md).
...
2. Admission cache до сети и bounded parallel probes через существующий `WorkersService`. **Сделано как Stage 5.2 на `admission-cache-20260909` (парные измерения 8f10ed2→7519441; полный suite на 0745f2b: 1794 passed / 0 failed / 0 ignored, 68 Running); этап 5 целиком открыт.**
3. Прямой lookup task по ID (**сделано ранее**); пропуск no-op записей и отсутствие перекрывающихся snapshot/detail polls (**сделано как Stage 5.3a на `915227f`**); сбор observations независимо от HTTP-запроса (**открыто**). Этап 5 целиком открыт.
