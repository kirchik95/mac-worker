# Getting started with mac-worker

For the short version, see [English](../README.md) or [Русский](../README.ru.md). This guide covers installation options, laptop agent skills, task execution, and updates.
...
<p align="center">
  <img src="images/dashboard.png" alt="Dashboard showing connected Macs, queued tasks and completed work" width="100%">
</p>
...
Use the worker's hostname or IP address; an existing SSH alias also works. If SSH keys are not set up, follow [SSH preparation](setup-macos-worker.md#2-connect-over-ssh).
...
By default your laptop coordinates the work (local queue, scheduler, dashboard). The selected Mac runs the agent and keeps its task workspace. This diagram shows that default flow, using a local repository as the source. Leave `[controller]` unset, or set `enabled = false`, for this mode. An always-on remote controller is opt-in and off until you enable it; see [Remote controller](usage.md#remote-controller).
...
To remove the CLI installed by the script, delete `~/.local/bin/worker` (or the file in your chosen `--bin-dir`). This keeps configuration and task history. See [removal and stored data](setup-macos-worker.md#removal-and-stored-data) before retiring a worker.
...
- [Snapshot batching](2026-09-09-snapshot-batch-performance.md): bounded Git work for `--wip` capture.
- [Snapshot and dashboard lookup](2026-09-09-performance-improvements.md): faster snapshot prep and per-task detail/log reads.
- [Pool reliability roadmap](superpowers/plans/2026-09-08-pool-reliability-roadmap.md): completed and upcoming reliability and performance work.
- [Design notes](superpowers/specs/) and [implementation plans](superpowers/plans/).
