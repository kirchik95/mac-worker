...
Top-level keys are defaults. A task may override them. Validate every entry before submitting the batch. The run groups them for status, waiting, and `max_parallel`.
