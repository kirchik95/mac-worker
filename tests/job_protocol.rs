...
fn material_rejects_noncanonical_identifiers_and_duplicate_json_fields() {
    ...
    assert!(serde_json::from_str::<RequestFingerprintMaterial>(&format!(
        r#"{{"protocol_version":6,"protocol_version":6,"job_id":"{JOB_ID}","client_id":"{CLIENT_ID}","lease_token":"{LEASE_TOKEN}","worker_name":"mini-1","project_id":"{PROJECT_ID}","worktree_id":"{WORKTREE_ID}","manifest_digest":"{MANIFEST_DIGEST}","relative_working_dir":"","timeout_millis":1,"resource_class":"heavy","command":{{"mode":"shell","shell":"true"}}}}"#
    )).is_err());
}
...
fn streaming_events_are_versioned_strict_ndjson_records() {
    ...
    .is_err());
    assert!(serde_json::from_str::<JsonEvent>(
        &format!(r#"{{"event":"error","protocol_version":{PROTOCOL_VERSION},"code":"A","code":"B","message":"worker is busy"}}"#)
    )
    .is_err());
}
...
fn task8_query_dtos_reject_missing_unknown_duplicate_trailing_and_invalid_values() {
    ...
    for invalid in [
        ...
        r#"{"protocol_version":6,"job_id":"BAD"}"#.to_string(),
        format!(r#"{{"protocol_version":6,"job_id":"{JOB_ID}","extra":true}}"#),
        format!(r#"{{"protocol_version":6,"protocol_version":6,"job_id":"{JOB_ID}"}}"#),
    ...
    for invalid in [
        format!(
            r#"{{"protocol_version":6,"job_id":"{JOB_ID}","stream":"stdout","offset":0,"limit":-1}}"#
        ),
        format!(
            r#"{{"protocol_version":6,"job_id":"{JOB_ID}","stream":"stdout","offset":0,"limit":4294967296}}"#
        ),
        format!(
            r#"{{"protocol_version":6,"job_id":"{JOB_ID}","stream":"stdin","offset":0,"limit":1}}"#
        ),
        format!(
            r#"{{"protocol_version":6,"job_id":"{JOB_ID}","stream":"stdout","offset":0,"limit":1,"limit":2}}"#
        ),
    ] {
        assert!(
...
fn resolution_outcomes_uncertainty_and_host_errors_are_strict_and_bounded() {
    ...
    assert!(
        serde_json::from_str::<ResolveOrAbandonResponse>(
            r#"{"protocol_version":6,"outcome":"abandoned","response":null}"#
        )
        .is_err()
    );
    assert!(
    ...
    assert!(
        serde_json::from_str::<HostControlError>(
            r#"{"protocol_version":6,"error":{"code":"A","code":"B","message":"m"}}"#
        )
        .is_err()
    );
}
